...
def city_demand_plot(which_city, conn):
    ...
    # Read the query result into a DataFrame
    df = pd.read_sql_query(query, conn, params=(which_city,))
    ...
    plt.xlabel("Date")
    plt.ylabel("Total Demand")
    plt.title(f"Total Demand in {which_city}")
    plt.grid(True)
    plt.tight_layout()
...
def main():
    ...
    total_demand_plot(conn)
    city_demand_plot("Montreal", conn)
